...
import type {
    ...
    SerializableSecretlintLintOptions,
...
const lintResultCache = new Map<string, SerializableSecretlintResult>();
let secretlintWorker: null | Worker = null;
const isUsesTypeScriptSourceWorker = import.meta.url.endsWith(".ts");
const workerModuleUrl = new URL(
    isUsesTypeScriptSourceWorker
        ? "./secretlint-worker.ts"
        : "./secretlint-worker.js",
    import.meta.url
);

const createWorker = (): Worker =>
    new Worker(workerModuleUrl, {
        name: "secretlint-eslint-bridge",
        ...(isUsesTypeScriptSourceWorker && {
...
const resetWorker = (): void => {
    const workerToTerminate = secretlintWorker;
    ...
    secretlintWorker = null;
};

const getWorker = (): Worker => {
    if (secretlintWorker === null) {
        secretlintWorker = createWorker();
        secretlintWorker.unref();
        secretlintWorker.once("error", () => {
            secretlintWorker = null;
        });
        secretlintWorker.once("exit", () => {
            secretlintWorker = null;
        });
    }
    return secretlintWorker;
};

const readWorkerResponse = (
    response: SecretlintWorkerResponse | undefined
): SerializableSecretlintResult => {
...
export const runSecretlintSynchronously = (
    ...
    if (waitResult === "timed-out") {
        ...
        resetWorker();
        throw new Error(
            "Timed out while waiting for the Secretlint worker to finish."
        );
    }
    if (Atomics.load(signal, 0) !== WORKER_DONE_STATE) {
        port1.close();
        throw new Error("Secretlint worker did not enter a completed state.");
    }
    const workerMessage = receiveMessageOnPort(port1);
    port1.close();
    const result = readWorkerResponse(
        safeCastTo<SecretlintWorkerResponse | undefined>(workerMessage?.message)
    );
