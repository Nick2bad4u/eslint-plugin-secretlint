...
Prefer nested rules array keeps secretlint configuration files predictable when they are reviewed beside ESLint configuration.
...
- Files matched by the bridge rule or the configuration preset that enables `secretlint/prefer-secretlint-nested-rules-array`.
...
export default [
 ...
 {
  rules: {
   "secretlint/prefer-secretlint-nested-rules-array": "error",
  },
 },
];
```
