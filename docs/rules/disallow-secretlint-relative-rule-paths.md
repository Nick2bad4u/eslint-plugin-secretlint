...
Disallow relative rule paths keeps secretlint configuration files predictable when they are reviewed beside ESLint configuration.
...
- Files matched by the bridge rule or the configuration preset that enables `secretlint/disallow-secretlint-relative-rule-paths`.
...
The rule reports relative local rule paths that make secretlint config harder to run consistently from ESLint.
...
.secretlintrc.json
rules:
  - id: @scope/secretlint-rule-no-token
```
...
export default [
 ...
 {
  rules: {
   "secretlint/disallow-secretlint-relative-rule-paths": "error",
  },
 },
];
```
