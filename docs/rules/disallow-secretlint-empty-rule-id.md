...
Disallow empty rule id keeps secretlint configuration files predictable when they are reviewed beside ESLint configuration.
...
- Files matched by the bridge rule or the configuration preset that enables `secretlint/disallow-secretlint-empty-rule-id`.
...
The rule reports empty arrays, empty objects, or blank pattern lists that look intentional but do not configure useful secretlint behavior.
...
export default [
 ...
 {
  rules: {
   "secretlint/disallow-secretlint-empty-rule-id": "error",
  },
 },
];
```
