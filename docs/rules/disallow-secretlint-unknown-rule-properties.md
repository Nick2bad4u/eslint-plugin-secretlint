...
Disallow unknown rule properties keeps secretlint configuration files predictable when they are reviewed beside ESLint configuration.
...
- Files matched by the bridge rule or the configuration preset that enables `secretlint/disallow-secretlint-unknown-rule-properties`.
...
The rule reports unknown or misspelled properties in secretlint configuration objects before they silently drift from the upstream schema.
...
export default [
 ...
 {
  rules: {
   "secretlint/disallow-secretlint-unknown-rule-properties": "error",
  },
 },
];
```
