...
Require rules packages installed keeps secretlint configuration files predictable when they are reviewed beside ESLint configuration.
...
- Files matched by the bridge rule or the configuration preset that enables `secretlint/require-secretlint-rules-packages-installed`.
...
The rule reports referenced secretlint rule packages that are missing from project dependencies.
...
```yaml
npm install --save-dev @secretlint/secretlint-rule-example
```
...
export default [
 ...
 {
  rules: {
   "secretlint/require-secretlint-rules-packages-installed": "error",
  },
 },
];
```
