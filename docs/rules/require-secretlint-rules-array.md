...
Require rules array keeps secretlint configuration files predictable when they are reviewed beside ESLint configuration.
...
- Files matched by the bridge rule or the configuration preset that enables `secretlint/require-secretlint-rules-array`.
...
The rule reports missing required secretlint config structure for require rules array.
...
export default [
 ...
 {
  rules: {
   "secretlint/require-secretlint-rules-array": "error",
  },
 },
];
```
